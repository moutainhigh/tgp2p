...
import com.tpy.p2p.chesdai.spring.service.VerificationService;
	...
	public Object invoke(MethodInvocation mi) throws Throwable {
		...
		if (bool) {
			...
			if (user == null) {
				// toLogin(out);
				/*
				 * request.setAttribute(Constant.SECURITY_VERIFIY,
				 * "您还没有登录，请先登录！"); 判断app是否有uid的值
				 */
				String uid = request.getParameter("uid");
				if (null != uid) {
					JSONObject result = new JSONObject();
					user = memberCenterService.queryById(Long.decode(uid));
					int msg = verificationService.queryVerification(user);
					if (msg <= 5) {
						// toDialog(request, out,
						// "verification/check_fund_safe?msg=0");
						return result.accumulate(Constant.SECURITY_VERIFIY, msg);
					}
					if (msg >= 100) {
						request.setAttribute(Constant.SECURITY_VERIFIY, msg);
						return result
								.accumulate(Constant.SECURITY_VERIFIY, msg);
				...
				if (msg <= 5) {
					...
					// "verification/check_fund_safe?msg=0");
					request.setAttribute(Constant.SECURITY_VERIFIY, msg);
					return "/WEB-INF/views/member/memberCenter";
				}
				if (msg >= 100) {
					request.setAttribute(Constant.SECURITY_VERIFIY, msg);
					return "/WEB-INF/views/visitor/index";
