...
public class LoanrecordService {
    ...
    */
    public List queryPage(PageModel page,String state,String loantype) {
        ...
        if(null!=state&&state!=""){
            countsql.append(" and loanrecord.recordway=").append(StringUtil.replaceAll(state));
            listsql.append(" and loanrecord.recordway=").append(StringUtil.replaceAll(state));
        }
    ...
    */
    ...
    /**
     * <p>Title: queryLoanrecordList</p>
     * <p>Description:  通过借款标标号查询到该债权转让标的认购记录</p>
     * @param start start
     * @param limit limit
     * @param loanSignId 借款标id
     * @param num  如果=1 就加limit 如果!=1就不加
