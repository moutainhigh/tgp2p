package com.tpy.p2p.chesdai.spring.service.invest;
...
import com.tpy.p2p.chesdai.entity.CalculateLoan;
...
import com.tpy.p2p.chesdai.util.BasicData;
...
public class InvestService {
	...
	public List<Loanrecord> getLoanRecord(int state, Long userid, int no,
		...
		if (states == 1) {
			...
			sql = "select * from loanrecord dlr where dlr.userbasicinfo_id="
					+ userid + " and dlr.Loansign_id in (select ls.id from "
					+ " Loansign ls,loansignbasics lsb where ls.loanstate=" + state
					+ " and lsb.creditTime is NOT NULL)";
		}
		if (0 == state) {
			sql = "select * from loanrecord dlr where dlr.userbasicinfo_id="
					+ userid;
	...
	public List<Object> getLoanGlF(List<Object> loanlist) throws ParseException {
		...
		for (int i = 0; i < loanlist.size(); i++) {
			...
			loannew.setLoanNumber(loanRecord.getLoansign().getLoansignbasics()
					.getLoanNumber());
			loannew.setLoanId(Loansign.getId());
			// 借款标题
			loannew.setLoanTitle(loanRecord.getLoansign().getLoansignbasics()
					.getLoanTitle());

			// 判断是否是进行中的标
			if (Loansign.getLoanstate() == 2 && null != Loansign.getPublishTime()) {

                String loanday=DateUtils.add("yyyy-MM-dd HH:mm:ss",Loansign.getPublishTime(),Calendar.DATE,Loansign.getLoansignbasics().getBidTime() );
				int days = DateUtils.differenceDate("yyyy-MM-dd",
						DateUtils.format("yyyy-MM-dd"), loanday);
				loannew.setLastDays(days > 0 ? days : 0);// 剩余标期
			}
			...
			// 出借单位
			loannew.setLoanUnit(Loansign.getLoanUnit());
			// 借款金额
...
//					
//					 loannew.setPrincipalandInterest(
//							 Arith.reimbursement(loanRecord.getLoansign(), loanRecord.getLoansign().getMonth()) +
//							 //Arith.reimbursement(loanRecord.getLoansign(), loanRecord.getLoansign().getMonth()) loanRecord.getLoansign().getMonth() * bl +
//					  "");
//					 } else {
...
//				}
	 ...
	 * @return
	 */
	public List<Loanrecord> getLoanRecordTwo(int state, Long userid, int no,
			int states) {
		// 根据用户和标状态查新投资标信息
		String sql = "select * from loanrecord dlr where dlr.userbasicinfo_id="
				+ userid
				...
				+ " Loansign ls where ls.loanstate="
				+ state
				+ " and ls.loanType!=6 and ls.loanType!=7) and dlr.loan_id is null order by dlr.id desc ";
		sql = sql + " LIMIT " + no + ",10";
		List<Loanrecord> loanrecordlist = dao.findBySql(sql, Loanrecord.class);
	...
	public double getSumTenderMoney(List<Object> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			double m = Double.valueOf(((LoanNew) list.get(i)).getTenderMoney());
			// 计算完成的金额
	...
	public double getMoney(List<Object> list) {
		double money = 0;
		for (int i = 0; i < list.size(); i++) {
			LoanNew loanNew = (LoanNew) list.get(i);
			String pli = loanNew.getPrincipalandInterest();
			double m = 0;
			if (null != pli && !(pli.equals(""))) {
				m = Double.valueOf(pli);
			}
			// 计算完成的金额
			money = money + Double.valueOf(m);
		}
	...
	public List<Object[]> getRepay(Long userId, int no) {
		StringBuffer str = new StringBuffer(
				"SELECT b.rate,b.issueLoan,b.loanType,(SELECT loanNumber FROM loansignbasics WHERE id = b.id),"
						...
						+ "(SELECT loanTitle FROM loansignbasics WHERE id = b.id) FROM");
		str.append(" loanrecord a INNER JOIN Loansign b ON a.Loansign_id = b.id INNER JOIN Repaymentrecord c ON b.id=c.Loansign_id INNER JOIN userbasicsinfo u ON u.id=b.userbasicinfo_id");
		str.append(" WHERE  a.userbasicinfo_id = ");
		str.append(userId);
		str.append("  AND c.repayTime IS NULL AND c.repayState=1 AND  ( b.loanState = 3 OR(b.loanType = 4 AND b.loanState != 4))");
	...
	public LoanUncollected getLoanUncollected(BasicData obj) {
		...
		if (obj.getLoanType() == 1) {
			if (obj.getRefundWay() == 1) {
				...
				for (int i = 0; i < calcu.size(); i++) {
					CalculateLoan c = calcu.get(i);
					if (obj.getPeriods() == i + 1) {
		...
		// 应收本金
		loanun.setTenderMoney(principalReceivables);
		// 应收利息
		...
		if (obj.getIsPrivilege() == 1) {// 会员
			...
			if (managementFee > obj.getPmfeetop()) {
				managementFee = obj.getPmfeetop();
			}
		} else {// 非会员
			managementFee = interest * obj.getMfeeratio();
			if (managementFee > obj.getMfeetop()) {
				managementFee = obj.getMfeetop();
			}
		}
	...
	public List<LoanUncollected> getLentDetails(List<Loanrecord> loanlist) {
		...
		for (int i = 0; i < loanlist.size(); i++) {
			...
			String sql1 = "SELECT SUM(r.money) AS c, SUM(r.preRepayMoney) AS d FROM repaymentrecord r WHERE r.loanSign_id = ? AND r.repayState != 1 AND r.repayState != 3";
			// 获取未收
				...
				if (loan.getLoansign().getLoanType()
					...
					if (loan.getLoansign().getRefundWay()
						...
						daishou = signFund.instalmentInterest(
								...
								Constant.STATUES_ONE).doubleValue();
