...
import com.tpy.p2p.chesdai.constant.enums.ENUM_COLUMN_ISFIXED;
...
public class ColumnManageService {
    ...
    /**
     * 查询所有的提纲，并将其存放到application中
     * 
     * @param application
     *            ServletContext
     */
    public void queryAllOutline(ServletContext application) {
    	List<Outline> list = commondao.find(
                "FROM Outline a order by a.id asc");
        application.setAttribute("outlines", list);
    }
    ...
    public void queryAllArticles(ServletContext application) {
        List<Article> list = commondao.find(
                "FROM Article a where a.isRecommend = 1 order by a.id asc");
    ...
    public void resetApplaction(HttpServletRequest request){
        ...
        queryAllArticles(request.getSession()
                .getServletContext());
        queryAllOutline(request.getSession()
        		.getServletContext());
    }
