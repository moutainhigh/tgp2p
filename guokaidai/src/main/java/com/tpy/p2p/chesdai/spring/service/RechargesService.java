...
public class RechargesService {
    ...
    public List<Recharge> rechargeList(Long id, String beginTime, String endTime,PageModel page) {
        ...
        if (null != beginTime && !"".equals(beginTime)) {
            hql.append(" and date_format(r.time,'%Y-%m-%d')>='").append(StringUtil.replaceAll(beginTime)+"'");
        }

        if (null != endTime && !"".equals(endTime)) {
            hql.append(" and date_format(r.time,'%Y-%m-%d')<='").append(StringUtil.replaceAll(endTime)+"'");
        }
        hql.append(" ORDER BY r.time DESC LIMIT ").append(page.firstResult()+","+page.getNumPerPage());
    ...
    public Recharge getRecharge(String orderNum) {
        ...
        StringBuffer hql = new StringBuffer("from Recharge r where r.orderNum=")
                .append(StringUtil.replaceAll(orderNum));
    ...
    public Map<String, String> encryption(RechargeInfo recharge)
        ...
        // 将充值信息转换成xml文件
        String registerCall = com.tpy.p2p.pay.util.ParseXML.rechargeXml(recharge);
