...
import com.tpy.p2p.chesdai.admin.spring.service.BorrowersApplyService;
import com.tpy.p2p.chesdai.admin.spring.service.IntegralService;
...
public class UserInfoController {
    ...
    public JSONObject queryPage(Userbasicsinfo userinfo, PageModel page,
        ...
        String titles = "id,username,realname,phone,email,suminte,credit,createTime,lasttime,lastaddress,logincount,vipendtime,isborr,signAutoRepay,isLock,adminname";
        // 将查询结果转换为json结果集
    ...
    public JSONObject queryAssignmentPage(Userbasicsinfo userinfo, PageModel page,
            HttpServletRequest request, String limit, String start) {
        if (LOGGER.isDebugEnabled()) {
    ...
    public String queryAssignmentLoanSign(String loanId, HttpServletRequest request) {

        if (StringUtil.isNotBlank(loanId) && StringUtil.isNumberString(loanId)) {

            List<Object> objlist = userInfoServices.queryAssignmentLoanSign(loanId);
	...
	public JSONObject updateStatus(String id,int num){
		...
		try {
			...
			if(num==0){
				...
				//auditStatus=3 已审核
				//AuditResult=1 通过
				if(assginService.updateSql(id, 3, 1)){
					return DwzResponseUtil.setJson(json, Constant.HTTP_STATUSCODE_SUCCESS, "审批成功", assignmentId, null);
				}else{
					return DwzResponseUtil.setJson(json, Constant.HTTP_STATUSCODE_ERROR, "审批失败", assignmentId, null);
				}
			}else if(num==1){	
				//已经审核，不通过
				//auditStatus=3 已审核
    ...
    public JSONObject addUserBasic(Userrelationinfo relation, String userName,
        ...
        try {
            ...
            Userbasicsinfo userbasic = registrationService.registrationSave(
                    userName, relation.getPhone(), password, null, null, 1,
                    null, request);
    ...
    */
    ...
    public JSONObject queryApply(String limit, String start, PageModel page){
          ...
          if(applyList!=null){
        	  for (BorrowersApply apply : applyList) {
                  ...
                  case 1:
                      json.element("type", "太平洋宝");
                      break;
                  case 2:
                      json.element("type", "转盈宝");
                      break;
                /*  case 3:
    ...
    public JSONObject saveOrUpdatemanualIntegration(Manualintegral manualin) {
        ...
        if(manualintegral != null){
        	amountpoints += manualintegral.getTgPoints() != null ? manualintegral.getTgPoints() : 0;
        }
    ...
    */
    ...
    public JSONObject adminuserMessages(String start, String limit,PageModel page,HttpServletRequest request){
    	Adminuser user = (Adminuser) request.getSession().getAttribute(Constant.ADMINLOGIN_SUCCESS);
		...
		// 分页数据源
		List list = messagesettingService.getMessages(user.getId().toString(),page);
