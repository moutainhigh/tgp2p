package com.tpy.p2p.chesdai.spring.service.sign;
...
public class SignSql {
    ...
    public static String querySql(int type,String title,String grade,String name,String state,String areLenders,String beginTime,String endTime){
    	...
    	if(Validate.emptyStringValidate(title)){
    		sql.append(" and b.loanTitle=").append(StringUtil.replaceAll(title));
    	}
    	if(Validate.emptyStringValidate(grade)){
    		sql.append(" and b.loanNumber=").append(StringUtil.replaceAll(grade));
    	}
    	if(Validate.emptyStringValidate(name)){
    		sql.append(" c.`name`=").append(StringUtil.replaceAll(name));
    	}
...
//    	}
    	if(Validate.emptyStringValidate(beginTime)){
    		sql.append(" and TO_DAYS(a.publishTime) - TO_DAYS('").append(StringUtil.replaceAll(beginTime)).append("') >=0");
    	}
    	if(Validate.emptyStringValidate(endTime)){
    		sql.append(" and TO_DAYS(a.publishTime) - TO_DAYS('").append(StringUtil.replaceAll(endTime)).append("') <=0");
    	}
     	return sql.toString();
    }
    /**
