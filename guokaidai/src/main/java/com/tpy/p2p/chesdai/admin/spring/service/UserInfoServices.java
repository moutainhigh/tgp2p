...
import com.tpy.p2p.chesdai.util.SQLUtils;
...
import com.tpy.p2p.chesdai.spring.service.MyMoneyService;
import com.tpy.p2p.chesdai.util.SQLUtils;
import com.tpy.p2p.core.listener.UserList;
...
public class UserInfoServices {
    ...
    public List queryUserPage(PageModel page, Userbasicsinfo userinfo) {
        ...
        countsql.append(" WHERE userbasicsinfo.isespecialuser is null and userbasicsinfo.userName is not null ");

        // 查询数据sql拼接
        StringBuffer sqlbuffer = new StringBuffer(
                "SELECT userbasicsinfo.id, userbasicsinfo.userName, userbasicsinfo.`name`, userrelationinfo.phone, userrelationinfo.email, ");
        sqlbuffer
        ...
        sqlbuffer
                .append(" LEFT JOIN userrelationinfo ON userrelationinfo.user_id = userbasicsinfo.id WHERE userbasicsinfo.isespecialuser is null and userbasicsinfo.userName is not null ");
    ...
    public List queryAssignmentLoanSign(String ids) {
        ...
        sqlbuffer
                .append("where a.id=c.id and c.userbasicinfo_id=b.id and a.id="+Long.parseLong(ids));
    ...
    public void saveUser(Userbasicsinfo basic){
    	
    	dao.save(basic);
    }
    
   public void saveUser1(Userrelationinfo user){
    	
    	dao.save(user);
    }
   
   public void saveUserFun(Userfundinfo user){
   	
   	dao.save(user);
   }
   
   public void saveUsermess(Usermessage user){
   	
   	dao.save(user);
   }
   
   public void saveAuto(Automatic auto){
    ...
    * <p>Description: 未读短信数量、统计会员总数、特权会员数量、普通会员数量</p>
     * @param adminid 
    * @return 会员总数,特权会员数量,普通会员数量、在线会员数量
    */
    public String getUsercount(String adminid){
    	//查询该后台服务人员相关的短信条数
        String sql="SELECT COUNT(id) FROM adminmessage WHERE adminuser_id LIKE '%"+adminid+"%' and isread=0";
        ...
        return unRead_msgs+","+sum_count+","+vip_count+","+(sum_count-vip_count)+","+ul.getUserCount();
    ...
    */
    ...
    public List queryPage(PageModel page, Userbasicsinfo userinfo) {
        ...
        // 查询数据sql
        StringBuffer sqlbuffer = new StringBuffer(
                "select c.id,b.flowid ,e.userName ,e.name,d.loanTitle,d.loanNumber,c.loanState ,b.auditStatus,b.auditResult,b.tenderMoney ,a.userbasicinfo_id ,b.loan_id,b.shift_time,b.user_auth,b.user_debt from loanrecord ");
        sqlbuffer
                .append("  a ,loansignflow b,loansign c,loansignbasics d,userbasicsinfo e where a.loanSign_id=b.loan_id and a.userbasicinfo_id=b.user_debt and c.id=d.id and a.loanSign_id=c.id and b.user_debt=e.id order by c.id desc ");

        // 统计数据条数
        StringBuffer countsql = new StringBuffer(
                "SELECT COUNT(1) from loanrecord a ,loansignflow b,loansign c,loansignbasics d,userbasicsinfo e where a.loanSign_id=b.loan_id and a.userbasicinfo_id=b.user_debt and c.id=d.id and a.loanSign_id=c.id and b.user_debt=e.id");
    ...
    public List queryBasinfo(String userId) {
        ...
        sqlbuffer.append("WHEN (SELECT vipinfo.endtime FROM vipinfo WHERE vipinfo.user_id=userbasicsinfo.id ORDER BY id DESC LIMIT 1)");
        sqlbuffer
