...
public class VerificationService {
     ...
     * @param id
     *            用户编号
     * @return 1、邮箱验证, 2、实名认证, 3、安全问题 ,4、手机验证，5 没注册环讯   其余通过验证
     */
    public Integer queryVerification(Userbasicsinfo user) {
        // 判断手机是否通过验证
        String phone = user.getUserrelationinfo().getPhone() == null ? "" : user.getUserrelationinfo().getPhone().trim();
        if (phone.equals("")) {
            return 1;
            //return 2;   //先跳过手机验证，直接跳转到邮箱验证
        }
        Integer email = user.getUserrelationinfo().getEmailisPass() == null ? 0  : user.getUserrelationinfo().getEmailisPass();
        // 如果没有通过邮箱验证
        /*
        if (email == 0) {
            return 2;
        ...
        String cardId = user.getUserrelationinfo().getCardId() == null ? "" : user.getUserrelationinfo().getCardId().trim();
        // 实名验证没有通过
        ...
        //环迅注册帐号
        /*
        String ipsAccount=user.getUserfundinfo().getpIdentNo()==null?"":user.getUserfundinfo().getpIdentNo();
        //判断是否注册过环迅
        if(ipsAccount.equals("")){
        	return 5;
        }*/
        //判断用户是否设置交易密码
        if(user.getTransPassword()==null || user.getTransPassword().equals("")){
            return 5;
        }
