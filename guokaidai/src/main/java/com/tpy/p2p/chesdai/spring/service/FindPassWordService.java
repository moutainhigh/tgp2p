...
public class FindPassWordService {
	...
	/**
	 * <p>
	 * Title: queryUserlationBysome
	 * </p>
	 * <p>
	 * Description:通过电话号码或邮箱找到用户的联系信息
	 * </p>
	 * 
	 * @param phone
	 *            电话
	...
	public Userrelationinfo queryUserlationBysome(String phone, int state) {
		StringBuffer querysql = new StringBuffer(
				"SELECT * from userrelationinfo where ");

		if (state == 1) {
			querysql.append("phone=").append(StringUtil.replaceAll(phone));
		} else {
			querysql.append("email='").append(StringUtil.replaceAll(phone)).append("'");
		}

		List<Userrelationinfo> userrelalist = commonDao.findBySql(
				querysql.toString(), Userrelationinfo.class);
	 ...
	 *             异常
	 */
	public void sendEmail(Userbasicsinfo user, String email, Integer code,
	...
	public Integer sendEmailCodel(Userbasicsinfo user, String email,
			HttpServletRequest request) throws IOException, TemplateException {
		...
		// 邮件验证码
		Integer code = StringUtil.getvalidcode();
	 ...
	 */
	public Integer sendsesCodel(Userbasicsinfo user, String phone,
		...
		if (null == user.getUserName() && null != user.getNickname()) {
