...
public class RegistrationController {
	...
	public List checkOnlyPhone(String fieldId, String fieldValue) {
		...
		list.add(fieldId);
		// 验证邮箱是否唯一
		boolean bool = registrationService.checkPhone(fieldValue);
		list.add(bool);
		return list;
	}
	...
	public String registrationMethod(String userName, String phone, String character, String pwd,
			String captcha, String number,String recommend, HttpServletRequest request) {
		...
		try {
			// 取验证码
			String validate = (String) request.getSession().getAttribute("registration");
			// 判断验证码是否正确
			if (validate != null && validate.equalsIgnoreCase(captcha)
					&& !StringUtil.isBlank(userName)
					&& !StringUtil.isBlank(phone) && !StringUtil.isBlank(pwd)) {
				...
				Userbasicsinfo isToPromoter = registrationService
						.registrationSave(userName, phone, pwd, number,recommend,Integer.parseInt(character),
								promoter, request);
				// 注册成功
	...
	//TODO 是否注册时即发送短信？
	/**
	 * 填写完资料，马上发短信？涉及到userrelation的phone字段，如果该字段不为空，而手机号码不匹配，会导致手机验证跳过。
	 * @param phone
	 * @param request
	 * @return
	 * @throws Exception
	...
	*/
	...
	public String ajaxLogin(String userName, String pwd, String captcha,
		...
		if (AccessTypeJudge.isAjax(request)) {
			...
			if (validate != null && validate.equalsIgnoreCase(captcha)) {
				...
				request.getSession().removeAttribute("user_login");
				// 验证登录是否成功
				Userbasicsinfo user = registrationService.loginMethod(userName,
						pwd);
				// 如果用户名、密码匹配
				if (user != null) {
					// 判断该会员是否被后台管理员禁用
					...
					if (!isLock) {
						...
						if (b) {
							...
							// 保存未读消息
							request.getSession().setAttribute("messagecount",
									obj);
						} else {
							// 如果还未过，保存时间
							request.setAttribute("isLock", user.getFailTime());
					...
					if (error > 0) {
						if (error == 5) {
							...
							// 保存错误次数
							result = "用户名或密码错误!今天您已输入错误" + error + "次，今天还可尝试:"
									+ (5 - error) + "次";
						}
					} else {
						// 用户名密码错误
	...
	public String activateAccount(Long activationid, HttpServletRequest request) {
		Integer identy = registrationService.activateAccount(activationid,
				request);
		request.setAttribute("url", "/member/mail");
		if (identy == 3) {
			return "/WEB-INF/views/reg_success";
		}
