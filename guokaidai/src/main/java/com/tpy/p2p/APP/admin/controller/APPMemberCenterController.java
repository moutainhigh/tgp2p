 ...
 */
@Controller
@RequestMapping(value = { "Appmember_index" })
@SuppressWarnings("rawtypes")
public class APPMemberCenterController {
	...
	/**
	 * 会员中心首页基本信息 直接传uid 跳过session
	 * 
	 * @param request
	 *            请求
	 * @return JSONObject
	 */
	@RequestMapping(value = "/member_center", method = RequestMethod.POST)
	@ResponseBody
	public JSONObject memberCenter(Long uid, HttpServletRequest request) {
		Map<String, Object> message = new HashMap<String, Object>();
		// 取到登录用户sesssion
		Userbasicsinfo user = queryUser(request);
		// 查询用户基本信息
		user = memberCenterService.queryById(uid);
		message.put("realName", user.getName());
		message.put("identityCard", user.getUserrelationinfo().getCardId());
		message.put("registerTime", user.getCreateTime());
		message.put("cashBalance", user.getUserfundinfo().getCashBalance());
		// 查询用户是否设置安全问题
		boolean bool = memberCenterService.isSecurityproblem(user.getId());
		message.put("safty", bool);
		// 是否为vip用户 null普通会员非null是vip会员
		Object isVip = vipInfoService.isVip(user.getId());
		message.put("isVip", isVip);
	...
	/**
	 * app 调用web接口 会员中心首页基本信息
	 * 
	 * @param request
	 *            请求
	 * @return 返回.jsp
	 */
	@RequestMapping(value = "/app-member_center", method = RequestMethod.POST)
	public ModelAndView memberCenter(HttpServletRequest request, Long uid) {
		// 取到登录用户sesssion
		...
		if (null != funding) {
			...
			if (null != funding.getpIdentNo()
				...
				BalanceInquiryInfo money = RegisterService
						.accountBalance(funding.getpIdentNo());
				// IPS账户余额
	 ...
	 * @return 返回.jsp
	 */
	@RequestMapping(value = "/system_message", method = RequestMethod.POST)
	@ResponseBody
	public JSONObject querySystemMessage(
		...
		// 查询用户系统消息
		List<Object[]> list = memberCenterService.queryUserMessage(
				user.getId(), page);
		// id,context,isread,receivetime,title
		...
		for (Object[] value : list) {
			...
			for (int x = 0; x < value.length; x++) {
				...
				case 0:
					...
					break;
				case 1:
					row.put("context", value[x]);
					break;
				case 2:
					row.put("isread", value[x]);
					break;
				case 3:
					row.put("receivetime", value[x]);
					break;
				case 4:
					row.put("title", value[x]);
					break;
		...
		// 查询用户登录日志
		List<Object[]> logs = memberCenterService.queryLog(user.getId());
		List<Object> logsLogin = new ArrayList<Object>();
		for (Object[] value : logs) {
			// id,logintime,ip,address
			Map<String, Object> row = new HashMap<String, Object>();
			for (int x = 0; x < value.length; x++) {
				if (null == value[x]) {
					value[x] = "";
				...
				case 0:
					...
					break;
				case 1:
					row.put("logintime", value[x]);
					break;
				case 2:
					row.put("ip", value[x]);
					break;
				case 3:
					row.put("address", value[x]);
					break;
		...
		all.put("sysMessage", sysMessage);
		all.put("logLogin", logsLogin);
		all.put("pageNum", page.getPageNum());
		all.put("numPerPage", page.getNumPerPage());
		all.put("totalCount", page.getTotalCount());
		all.put("totalPage", page.getTotalPage());
		all.put("count", Integer.parseInt(obj.toString()));
		all.put("messagecount", Integer.parseInt(obj.toString()));
		all.put("read", read);
		all.put("unRead",
