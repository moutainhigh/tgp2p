$(document).ready(function(){
	 ...
	 * 编辑标
	 */
	$("#editStaticLoan").click(function(){
		//取到选中记录
		var selections=$('#staticgrid1').omGrid('getSelections',true);
		//判断是否选中
	...
	$("#deleteAttachment").click(function(){
		//取到选中记录
		var selections=$('#staticgrid4').omGrid('getSelections',true);
		//判断是否选中
		if(selections==null || selections=="" || selections==undefined || selections.length<1){
			alertMsg.info("请选择信息！");
		}else{
			...
			$.ajax({
				...
				success:function(msg){
					if(msg){
						 $('#staticgrid4').omGrid('reload');//刷新当前页
	...
	var relaodFileTable=function (){
		$('#staticgrid4').omGrid('reload');
		return true;
	}
	/****************数据源**************/
	//加载出借记录列表
	var showstaticgrid2 = function(id){
		$("#staticgrid2").omGrid({
	...
	//还款记录
	var showstaticgrid3= function(id){
		$("#staticgrid3").omGrid({
	...
	//附件
	var showstaticgrid4= function(id){
		$("#staticgrid4").omGrid({
	...
	//评论
	var showstaticgrid5= function(id){
		$("#staticgrid5").omGrid({
	...
	$('#editComment').bind('click', function() {
        var selections=$('#staticgrid5').omGrid('getSelections',true);
    ...
    var submitDialog = function(){
        ...
        $.post('/comment/updateComment',submitData,function(msg){
        	if(msg){
        		$("#dialog-form").omDialog("close"); //关闭dialog
	              $('#staticgrid5').omGrid('reload');//如果是修改则刷新当前页
	...
	/***************按钮及点击事件************/
		...
		$("#FBloansign").click(function(){
            ...
            $.ajax({
    			...
    			success:function(msg){
    				if(msg==3){
    					 $('#staticgrid1').omGrid('reload');//刷新当前页数据
...
//    					 alertMsg.correct("标的结束！");
		...
		$("#exceedTimeRepay").click(function(){
			var selections=$('#staticgrid3').omGrid('getSelections',true);
            if (selections.length == 0) {
            	 alertMsg.info("请选择要还款的记录！");
                return false;
            }
            ...
            $.ajax({
    			...
    			success:function(msg){
    				if(msg==1){
    					 $('#staticgrid3').omGrid('reload');//刷新当前页数据
		...
		$("#advanceRepay").click(function(){
			var selections=$('#staticgrid3').omGrid('getSelections',true);
			var selections1=$('#staticgrid1').omGrid('getSelections',true);
            if (selections.length == 0) {
            	 alertMsg.info("请选择要还款的记录！");
                return false;
            }
	...
	showstaticgrid5(0);
	 ...
	 $("#loanSignTypeId").omCombo({
      ...
      $('span#static-search').omButton({
    	    icons : {left : '/resources/operamasks-ui/themes/default/images/search.png'},width : 70
 	 	});
      $('span#static-search').click(function(){
    	  $('#staticgrid1').omGrid({method:"post",extraData:$("#searchfrom").serializeObject()});
    	  $('#staticgrid1').omGrid('setData', '/staticloan/staticLoanList');

      });

	//数据源面板
	$('#staticgrid1').omGrid({
        dataSource : '/staticloan/staticLoanList',
        title : '借款标列表',
        height : 336,
        limit : 10,
        colModel : [ 
