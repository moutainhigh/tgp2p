$(document).ready(function(){
	...
	$("#deleteAttachment").click(function(){
		//取到选中记录
		var selections=$('#mygrid14').omGrid('getSelections',true);
		//判断是否选中
		if(selections==null || selections=="" || selections==undefined || selections.length<1){
			alertMsg.info("请选择信息！");
		}else{
	...
	var relaodFileTable=function (){
		$('#mygrid14').omGrid('reload');
		return true;
	}
	/****************数据源**************/
	
	//评论
	var showmygrid15= function(id){
		$("#mygrid15").omGrid({
	...
	$('#editComment').bind('click', function() {
        var selections=$('#mygrid15').omGrid('getSelections',true);
    ...
    var submitDialog = function(){
        ...
        $.post('/comment/updateComment',submitData,function(msg){
        	if(msg){
        		$("#dialog-form").omDialog("close"); //关闭dialog
	              $('#mygrid15').omGrid('reload');//如果是修改则刷新当前页
            ...
            $.ajax({
    			...
    			success:function(msg){
    				if(msg==0){
    					...
    					alertMsg.info("已经受理放款请求,请稍后尝试刷新页面！");
    				}else{
...
//    					 alertMsg.correct("标的结束！");
		...
		$("#exceedTimeRepay").click(function(){
			var selections=$('#mygrid13').omGrid('getSelections',true);
            if (selections.length == 0) {
            	 alertMsg.info("请选择要还款的记录！");
                return false;
            }
            ...
            $.ajax({
    			...
    			success:function(msg){
    				if(msg==1){
    					 $('#mygrid13').omGrid('reload');//刷新当前页数据
	 ...
	 $("#loanSignTypeId").omCombo({
      ...
      $('span#button-search').click(function(){
    	  $('#mygrid11').omGrid({method:"post",extraData:$("#searchfrom").serializeObject()});
    	  $('#mygrid11').omGrid('setData', '/loanSign/dcbLoanSignList');

      });

	//数据源面板
	$('#mygrid11').omGrid({
        dataSource : '/loanSign/dcbLoanSignList',
        title : '借款标列表',
        height : 720,
        limit : 24,
        colModel : [ 
